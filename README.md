...
#### 2.4 通信协议
因为整个系统中的很多都是基于TCP完成的，TCP是面向字节流的，所以通信中会出现粘包的问题。
解决粘包问题比较普遍的办法是：

1. 使用一个消息中不会用到的标志作为结束符，用来分隔完整的信息。比如HTTP中就使用\r\n作为结束符。
2. 使用协议通信。按照规定的协议进行通信，只有收到符合协议规范的消息时，才将其作为一条完整的信息。

在本系统中使用了协议通信的办法，协议格式：

|(int) msgid|(int) msglen|(string) msg|
|-|-|-|

一条完整的消息由消息头和消息内容组成，消息头中由msgid和msglen两个信息，msglen表示msg的长度，服务器根据msgid调用在对应的回调函数。msg是消息正文。
